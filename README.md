...
This project focuses on building and training a language model using
Long Short-Term Memory (LSTM) networks to generate coherent news
headlines. The dataset is preprocessed by selecting political news
headlines and tokenizing them. The model uses an embedding layer to
convert words into dense vectors, followed by LSTM layers that capture
sequential dependencies between words. Additionally, dropout is applied
to reduce overfitting, and a fully connected layer is used to predict
the next word in the sequence. The model is trained using CrossEntropy
loss and optimized with the Adam optimizer. During training, two
strategies are used to generate predictions: sampling with softmax,
which introduces stochasticity in the selction of most probable words to
follow, and argmax, which selects the most probable next word.
Furthermore, the project employs Truncated Backpropagation Through Time
(TBBTT) to improve training efficiency for longer sequences, allowing
...
1.  (2 pts) Download the data and read it as a pandas DataFrame. Filter
    only the news labeled \"Politics\" to obtain 35,602 sequences. Print
    and report the first three entries.

2.  (3 pts) Tokenize each title at the word level, convert all words to
    lowercase, and append the \<EOS\> token to each sequence. Save the
    tokenized list using the pickle format for future use. Print the
    first three tokenized sentences.

3.  (4 pts) Create a list, *all_words*, containing each unique word from
    the dataset, and include the \<EOS\> token at position 0 and the
    *PAD* token at the last position. Build two dictionaries:
    *word_to_int* and *int_to_word*, mapping words to integers and vice
    versa. Report the top 5 most common words.

4.  (5 pts) Implement a *Dataset* class. Each item should be a tuple:
    the first containing all words except the last, and the second
    containing all words except the first. Explain your implementation.

5.  (6 pts) Define a function *collate_fn(batch)* that pads sequences
    with the *PAD* token to the length of the longest sequence in the
    batch. Create a *DataLoader* that handles padding using
    *collate_fn*.

## Model Definition

Define your LSTM-based model with an embedding layer at the beginning
and LSTM layers (stacked if necessary). You may add Dropout, fully
connected layers, or any other components. Justify your design choices.
Ensure to include an *init_state()* method to initialize the model's
state. Discuss the main differences with RNNs.

## Evaluation - Part 1

1.  (3 pts) Implement a *random_sample_next* function that samples the
    next word based on $p(w_n|w_0, w_1, ..., w_{n-1})$.

2.  (4 pts) Implement a *sample_argmax* function that picks the word
    with the highest probability from $p(w_n|w_0, w_1, ..., w_{n-1})$.

3.  (3 pts) Implement a *sample* function that takes a prompt, the
    model, and one of the above sampling functions. The function
    generates a sentence until the \<EOS\> token is encountered.

## Training

1.  (15 pts) Train your model with a standard loop. After every epoch or
    every few epochs (as you choose), display a sentence generated by
    the model and the perplexity value. Stop training when the loss is
    below 1.5. Include:

    -   A plot of the loss function (show it falling below 1.5).

    -   A plot of perplexity values.

    -   Three generated sentences: one from the first epoch, one from
        the middle, and one from the last epoch. Provide commentary on
        the results.

2.  (20 pts) Implement Truncated Backpropagation Through Time (TBBTT) to
    manage long sequences by breaking them into chunks. Explain your
    modifications and train the model using TBBTT. Compare it with
    standard training. Include:

    -   A plot of the loss function (show it falling below 1).

    -   A plot of perplexity values.

    -   Three generated sentences: one from the first epoch, one from
        the middle, and one from the last epoch. Compare the results
        with standard training.

## Evaluation - Part 2

Generate 2-3 sentences using both sampling strategies:

-   Start with a prompt like "the president wants" and generate three
    sentences using the sampling strategy.

-   Start with the same prompt and generate three sentences using the
    greedy strategy.
